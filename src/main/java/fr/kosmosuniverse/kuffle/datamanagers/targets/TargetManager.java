...
public class TargetManager {
	 ...
	 *
	 */
	public static void setup(Mode mode, String content) {
		JSONObject allObj = FileUtils.readJSONObjectFromContent(content);
		targets = new HashMap<>();
		sbtts = new HashMap<>();
		
		setupVersions(mode, allObj);
		targetInv = new TargetInventories();
		targetInv.setupInventories(targets);
	}
	
	private static void setupVersions(Mode mode, JSONObject allObj) {
		for (String version : allObj.keySet()) {
			if (VersionManager.isAllowedVersion(version)) {
				...
				setupTypes(mode, versionObj);
			}
		}
	}
	
	private static void setupTypes(Mode mode, JSONObject versionObj) {
		for (String kuffleType : versionObj.keySet()) {
			if ("BOTH".equalsIgnoreCase(kuffleType) ||
					mode == Mode.valueOf(kuffleType.toUpperCase())) {
