...
public class Game implements Serializable {
	...
	public boolean skipPlayerTarget(boolean malus) {
		if (malus) {
			...
			if (targetCount > Config.getTargetPerAge()) {
				LogManager.getInstanceGame().writeMsg(player, LangManager.getMsgLang("CANT_SKIP_FINISHED", configLang));
	...
	public void nextPlayerAge() {
		...
		LogManager.getInstanceGame().logSystemMsg(LangManager.getMsgLang("AGE_VALIDATED", "en").replace("[#]", "[" + AgeManager.getAgeByNumber(age).getName() + "]").replace("<#>", "<" + player.getName() + ">"));
	...
	public String logString() {
		...
		sb.append(LangManager.getMsgLang("DEATH_COUNT", lang).replace("%i", String.valueOf(deathCount))).append("\n");
		sb.append(LangManager.getMsgLang("SKIP_COUNT", lang).replace("%i", String.valueOf(skipCount))).append("\n");
		sb.append(LangManager.getMsgLang("TEMPLATE_COUNT", lang).replace("%i", String.valueOf(sbttCount))).append("\n");
		...
		for (int i = 0; i < (Config.getLastAge().getNumber() + 1); i++) {
			...
			if (times.get(tmpAge.getName()) == -1) {
				...
				sb.append(LangManager.getMsgLang("ABANDON_AFTER", lang).replace("%s", tmpAge.getName().replace("_Age", "")).replace("%t", Utils.getTimeFromSec((times.get(tmpAge.getName()) * -1) / 1000))).append("\n");
				total += (times.get(tmpAge.getName()) * -1) / 1000;
				abandon = true;
			} else {
